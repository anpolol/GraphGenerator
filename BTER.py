...
class BTER():
    def __init__(self, N,degs,etta=0.1,ro=0.7,d_manual=0.75,betta=0.1):
        self.degrees = degs
        self.N = N
        self.etta = etta
        self.ro = ro
    ...
    def construct(self):
            G = nx.Graph()
            G.add_nodes_from(list(range(len( list(filter( lambda x: x>0,sorted(self.degrees)))))))
            ...
            if len(degrees)!=0:
                ...
                for i in (communities):
                    ...
                    if i != max(communities):
                        ro_r = self.ro*(1-self.etta*pow((np.log(comm[0]+1)/np.log(dmax+1)),2)) 
                        #ro_r=self.ro
                        #print('ro_r',ro_r)
            ...
            if sum(degs)>0:
                #print(excesses,degs)
                RandEdge = rv_discrete(l,r-1,values=(self.xke(l,r),self.pk_edge(degs)))
                ...
                tetta = 1-2*(int(np.round(ones))*min_deg/(int(np.round(ones))*min_deg+sum(excesses))) + self.betta
            ...
            #добавляем оставшиеся ребра CL моделью
                ...
                RandEdge_2 = rv_discrete(l,len(excesses)-1,values=(self.xke(l,len(excesses)),self.pk_edge(excesses[l:])))
    ...
    def func_excesses(self,x,tetta):
        if x>0: #>=2
            if tetta*x>=0:
                return tetta*x
            else:
