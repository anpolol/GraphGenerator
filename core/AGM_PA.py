...
def CreateLabelsAndAttr(network, prior=.5, conds=[[0.3, .4, .35], [.85, .65, .75]]): #тут учитывается корреляция меток и атрибутов
	labels = {}
	attrs = {}
	for vertex in network:
		labels[vertex] = int(random.random() < prior)
		attrs[vertex] = list(map(lambda x: int(random.random() < x), conds[labels[vertex]]))
	return labels, attrs
...
class SimpleBernoulliAR:
	...
	def set_accept_probs(self, ap=[1.0, .8, 1.0]):
